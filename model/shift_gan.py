...
class ShiftGAN(BaseVAE):
    ...
    def shift_img(self, decoded, shifts:torch.Tensor):
        # shifted = torch.zeros_like(decoded, device=decoded.device)

        # import pdb; pdb.set_trace()
        # N, _, H, W = decoded.size()
        # for i in range(N):
        #     # for x_out, x_org in zip(shifted, decoded):
        #     shifted[i, :, int(max(0, shifts[i][1])):int(min(H, shifts[i][1] + H)), int(max(0, shifts[i][0])):int(min(W, shifts[i][0] + W))] = \
        #     decoded[i, :, int(max(0, -shifts[i][1])):int(min(H, -shifts[i][1] + H)), int(max(0, -shifts[i][0])):int(min(W, -shifts[i][0] + W))]
