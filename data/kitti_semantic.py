...
class Kitti360Classes(Dataset):
	...
	def __getitem__(self, index):
		...
		voids = binary_classification[void_ids].sum(dim=0)
		flats = binary_classification[flat_ids].sum(dim=0)
		constructions = binary_classification[construction_ids].sum(dim=0)
		objects = binary_classification[object_ids].sum(dim=0)
		natures = binary_classification[nature_ids].sum(dim=0)
		sky = binary_classification[sky_ids].sum(dim=0)
		humans = binary_classification[human_ids].sum(dim=0)
